module ProfilesHelper
  ...
  def link_to_close_profile(profile)
    link_to icon('birthday-cake') + ' Lo studente ha <strong>terminato</strong> il servizio'.html_safe, 
  ...
  def link_to_resign_profile(profile)
    link_to icon('close') + ' Lo studente ha <strong>rinunciato</strong>'.html_safe, 
  ...
  def link_to_new_punch(profile)
    link_to icon('credit-card') + ' Inserisci timbratura', new_profile_punch_path(profile), class: :button
  end
end
